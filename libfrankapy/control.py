...
class SafetyLimits:
    ...
    max_cartesian_velocity: List[float] = field(default_factory=lambda: [1.7, 2.5, 2.5, 2.5, 2.5, 2.5])
    max_cartesian_acceleration: List[float] = field(default_factory=lambda: [13.0, 25.0, 25.0, 25.0, 25.0, 25.0])
...
class TrajectoryPoint:
    ...
    def __post_init__(self):
        ...
        if self.accelerations is not None and len(self.accelerations) != len(self.positions):
...
class MotionCommand:
    ...
    def __post_init__(self):
        ...
        if len(self.target_positions) not in [7, 6]:
            raise ValueError("target_positions must have 7 (joints) or 6 (Cartesian) values")
