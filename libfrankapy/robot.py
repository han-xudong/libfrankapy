...
from .state import JointState, CartesianPose, RobotState, ForceTorque
from .control import RealtimeConfig, ControlMode, MotionType, Trajectory
from .exceptions import (
    FrankaException, ConnectionError, ControlError, 
    SafetyError, TimeoutError, StateError
)
from .utils import (
    validate_joint_positions, validate_cartesian_pose,
    normalize_quaternion, create_home_position
)
...
class FrankaRobot:
    ...
    def move_to_joint(self, 
        ...
        try:
            ...
            positions_array = np.array(joint_positions, dtype=np.float64)
    ...
    def move_to_pose(self, 
        ...
        if len(target_pose) != 7:
            raise ValueError("target_pose must have exactly 7 elements [x, y, z, qx, qy, qz, qw]")
        ...
        motion_type_int = motion_type.value if hasattr(motion_type, 'value') else int(motion_type)
        
        # Normalize quaternion
        position = target_pose[:3]
        qx, qy, qz, qw = normalize_quaternion(*target_pose[3:])
        normalized_pose = position + [qx, qy, qz, qw]
        ...
        try:
            ...
            orientation_array = np.array([qx, qy, qz, qw], dtype=np.float64)
            
            success = self._controller.send_cartesian_position_command(
                position_array, orientation_array, speed_factor, motion_type_int, timeout
            )
            ...
            for i, point in enumerate(trajectory.points):
                ...
                if trajectory.is_joint_trajectory:
                    ...
                    if len(point.positions) == 6:
                        pose = point.positions[:3] + [0.0, 0.0, 0.0, 1.0]  # [x,y,z,qx,qy,qz,qw]
                    else:
