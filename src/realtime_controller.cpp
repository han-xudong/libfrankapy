...
RealtimeController::RealtimeController(const std::string& robot_ip)
    : robot_ip_(robot_ip) {
    std::cout << "RealtimeController created for robot: " << robot_ip_ << std::endl;
}
...
bool RealtimeController::connect() {
    ...
    try {
        ...
        handle_robot_error("Failed to connect to robot: " + std::string(e.what()), -1);
        return false;
...
bool RealtimeController::is_connected() const {
...
bool RealtimeController::start_control() {
    ...
    control_thread_ = std::make_unique<std::thread>(&RealtimeController::control_loop, this);
...
std::shared_ptr<SharedMemoryManager> RealtimeController::get_shared_memory() const {
...
void RealtimeController::update_shared_state(const franka::RobotState& robot_state) {
...
bool RealtimeController::process_control_commands(const franka::RobotState& robot_state) {
        ...
        switch (cmd_type) {
            case ControlMode::JOINT_POSITION: {
                ...
                execute_joint_position_control(
                    target_positions,
                    command.joint_speed_factor.load(),
            ...
            case ControlMode::CARTESIAN_POSITION: {
                ...
                execute_cartesian_position_control(
                    target_pose,
                    command.cartesian_speed_factor.load(),
...
void RealtimeController::execute_joint_position_control(
    const std::array<double, 7>& target_positions,
    double speed_factor,
    double acceleration_factor) {
    
    JointPositionMotionGenerator motion_generator(
        target_positions, speed_factor, acceleration_factor);
...
void RealtimeController::execute_cartesian_position_control(
    ...
    double qx = target_pose[3], qy = target_pose[4], qz = target_pose[5], qw = target_pose[6];
    ...
    CartesianPositionMotionGenerator motion_generator(
        target_transform, speed_factor, motion_type);
    
    robot_->control(motion_generator);
}

bool RealtimeController::check_safety_limits(const franka::RobotState& robot_state) {
    // Check joint limits
    ...
    for (size_t i = 0; i < 7; ++i) {
        if (robot_state.q[i] < joint_limits_lower[i] || 
            ...
            handle_robot_error("Joint limit violation on joint " + std::to_string(i), -10);
            return false;
        }
    }
    ...
    for (size_t i = 0; i < 7; ++i) {
        if (std::abs(robot_state.dq[i]) > max_joint_velocity) {
            handle_robot_error("Joint velocity limit violation on joint " + std::to_string(i), -11);
            return false;
        }
    }
...
void RealtimeController::handle_robot_error(const std::string& error_message, int error_code) {
    std::cerr << "Robot error [" << error_code << "]: " << error_message << std::endl;
...
bool RealtimeController::validate_control_command(const ControlCommand& command) {
