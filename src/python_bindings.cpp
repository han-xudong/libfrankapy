...
public:
    PyRealtimeController(const std::string& robot_ip) 
    ...
    bool is_connected() const {
        return controller_.is_connected();
    }
    
    bool start_control() {
        return controller_.start_control();
    }
    ...
    bool is_control_running() const {
        return controller_.is_control_running();
    }
    ...
    py::array_t<double> get_joint_positions() {
        ...
        auto positions = shared_memory_utils::get_joint_positions(shared_data->state.joint_positions);
    ...
    py::array_t<double> get_cartesian_position() {
        ...
        auto shared_data = shared_memory->get_shared_data();
        auto position = shared_memory_utils::get_cartesian_position(shared_data->state.cartesian_position);
    ...
    py::array_t<double> get_cartesian_orientation() {
        ...
        auto shared_data = shared_memory->get_shared_data();
        auto orientation = shared_memory_utils::get_quaternion(shared_data->state.cartesian_orientation);
    ...
    bool send_joint_position_command(
        const py::array_t<double>& target_positions,
    ...
    bool send_cartesian_position_command(
        ...
        const py::array_t<double>& target_orientation,
        double speed_factor = 0.1,
        ...
        if (target_orientation.size() != 4) {
            throw std::invalid_argument("target_orientation must have 4 elements (quaternion)");
        }
        ...
        static uint64_t command_counter = 0;
        command.command_id.store(++command_counter);
        command.command_type.store(static_cast<int>(ControlMode::CARTESIAN_POSITION));
...
public:
    ...
    bool is_connected() {
        return shared_memory_.is_initialized();
    }
    ...
    py::dict get_robot_state() {
        ...
        // Joint state
        std::vector<double> joint_positions(7), joint_velocities(7), joint_torques(7);
        for (size_t i = 0; i < 7; ++i) {
...
PYBIND11_MODULE(_libfrankapy_core, m) {
    ...
    py::class_<libfrankapy::PyRealtimeController>(m, "RealtimeController")
        ...
        .def("is_control_running", &libfrankapy::PyRealtimeController::is_control_running)
        .def("emergency_stop", &libfrankapy::PyRealtimeController::emergency_stop)
        
        // State queries
        .def("get_joint_positions", &libfrankapy::PyRealtimeController::get_joint_positions)
        .def("get_joint_velocities", &libfrankapy::PyRealtimeController::get_joint_velocities)
        .def("get_joint_torques", &libfrankapy::PyRealtimeController::get_joint_torques)
        .def("get_cartesian_position", &libfrankapy::PyRealtimeController::get_cartesian_position)
        .def("get_cartesian_orientation", &libfrankapy::PyRealtimeController::get_cartesian_orientation)
        .def("get_external_wrench", &libfrankapy::PyRealtimeController::get_external_wrench)
        ...
        .def("get_control_frequency", &libfrankapy::PyRealtimeController::get_control_frequency)
        ...
        .def("send_joint_position_command", 
             ...
             "target_positions"_a, "speed_factor"_a = 0.1, "acceleration_factor"_a = 0.1, "timeout"_a = 30.0)
        .def("send_cartesian_position_command",
             &libfrankapy::PyRealtimeController::send_cartesian_position_command,
             "target_position"_a, "target_orientation"_a, "speed_factor"_a = 0.1, "motion_type"_a = 0, "timeout"_a = 30.0)
    ...
    py::class_<libfrankapy::PySharedMemoryReader>(m, "SharedMemoryReader")
        ...
        .def("get_robot_state", &libfrankapy::PySharedMemoryReader::get_robot_state);
    
    // Utility functions
    m.def("get_current_timestamp", &libfrankapy::shared_memory_utils::get_current_timestamp);
    m.def("is_timestamp_recent", &libfrankapy::shared_memory_utils::is_timestamp_recent,
          "timestamp"_a, "timeout_us"_a = 1000000);
}