...
franka::JointPositions JointPositionMotionGenerator::operator()(
    ...
    if (!initialized_) {
        ...
        initialized_ = true;
        std::cout << "Joint motion initialized, duration: " << total_time_ << "s" << std::endl;
    }
    ...
    double s = 10 * std::pow(t_normalized, 3) - 15 * std::pow(t_normalized, 4) + 6 * std::pow(t_normalized, 5);
    
    std::array<double, 7> desired_positions;
    for (size_t i = 0; i < 7; ++i) {
        desired_positions[i] = initial_positions_[i] + s * (target_positions_[i] - initial_positions_[i]);
    }
...
franka::CartesianPose CartesianPositionMotionGenerator::operator()(
    ...
    if (!initialized_) {
        ...
        initialized_ = true;
        std::cout << "Cartesian motion initialized, duration: " << total_time_ << "s" << std::endl;
    }
    ...
    // Interpolate pose
    double t_normalized = time_ / total_time_;
    std::array<double, 16> desired_pose = interpolate_pose(initial_pose_, target_pose_, t_normalized);
...
std::array<double, 16> CartesianPositionMotionGenerator::interpolate_pose(
    const std::array<double, 16>& start,
...
double calculate_joint_distance(
    const std::array<double, 7>& pos1,
...
double calculate_cartesian_distance(
    const std::array<double, 16>& pose1,
