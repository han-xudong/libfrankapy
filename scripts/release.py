...
class ReleaseManager:
    ...
    def update_changelog(
        self, version: str, changes: Optional[List[str]] = None
    ) -> None:
        """Update changelog."""
...
"""
            ...
            new_content = content.replace(
                match.group(0), new_unreleased + release_section
            )
            ...
            print(
                "⚠️ Cannot find [Unreleased] section, please update CHANGELOG.md manually"
            )
    ...
    def release(
        ...
        if not dry_run:
            ...
            print("📝 Next steps:")
            print("   1. Push to remote repository: git push origin main --tags")
            print("   2. Publish to PyPI: python -m twine upload dist/*")
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run, no actual operations"
    )
    parser.add_argument("--skip-tests", action="store_true", help="Skip tests")
    parser.add_argument(
        "--current", action="store_true", help="Show current version number"
    )
